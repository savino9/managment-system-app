...
// Part 2
// Create two more routes:
// - route 4: renders a page with three forms on it (first name, last name, and email) that allows you 
...
reader.readFiles(jsonUser,  (contents) => {
	let newUser = '';
	let input = process.argv[2];

	for (var i = 0; i < contents.length; i++) {
		let name = contents[i].firstname;
		if (input === name && input !== undefined) {
			return console.log(`yeeah ${name}, you're in the server!`);
		} 
	}
	...
	for (var i = 0; i < contents.length; i++) {
		let name = contents[i].firstname;
		if (input !== name && input !== undefined) {
			return writer.writeFiles(contents, input, jsonUser);
		} 
	}
});
